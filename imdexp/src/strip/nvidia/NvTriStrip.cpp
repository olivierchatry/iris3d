					...
					PrimitiveGroup** primGroups, unsigned short* numGroups)
{
	...
	if(bListsOnly)
	{
		...
		*numGroups = 1;
		...
		for(uint i = 0; i < tempStrips.size(); i++)
		{
		...
		{
			for(uint j = 0; j < tempStrips[i]->m_faces.size(); j++)
			{
	...
	else
	{
		...
		*numGroups = numSeparateStrips; //for the strips
		...
		//first, the strips
		int startingLoc = 0;
		for(uint stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
		{
			int stripLength = 0;
			...
			if(!bStitchStrips)
			{
				...
				for(uint i = startingLoc; i < stripIndices.size(); i++)
				{
					if(stripIndices[i] == -1)
						break;
				}
		...
		if(tempFaces.size() != 0)
		{
			...
			primGroupArray[faceGroupLoc].numIndices = (int) tempFaces.size() * 3;
			int indexCtr = 0;
			for(uint i = 0; i < tempFaces.size(); i++)
			{
