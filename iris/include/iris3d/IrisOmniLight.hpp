...
*********************************************************/
#ifndef __OMNILIGHT_HPP__
#define __OMNILIGHT_HPP__

class IrisOmniLight : public IrisLight
{
protected:
	float	_radius;
public:	
	...
	virtual void		Compute(const vect3d &vin, const vect3d &normal, IrisColor &color, vertex2dtl *vout)
	{
		...
		if (result < 0.0f)
		{
			...
			color._r += (result * _color._r * 255.0f);
			color._g += (result * _color._g * 255.0f);
			color._b += (result * _color._b * 255.0f);
		}
